
<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<%@ page session="false"%>


<html>
    <head>
        <style><%@include file="/WEB-INF/resources/Home.css"%></style>
    </head>
    
    <body>
        <body background="http://lerevecafe.com/graphics/global/background.jpg">
    
   
        <header>
    
            <h1>Sam's Mediterranean Grill! </h1>

            <h4> Where great foods are made</h4>
        <nav>
            <ul>
                <h3>
                    <li><a href="orderOnline">Order</a></li>
                </h3>
                <h3><li>
                    <a href="contact">Contact</a></li>
                    </ul>
                </h3>
            </nav>
            <h3>Shawarma so GOOD, everyone love's it!</h3>
           <img src="https://cmkt-image-prd.global.ssl.fastly.net/0.1.0/ps/4584295/600/400/m2/fpnw/wm1/inujnigpqjb6rutovd98o9eyds3aqp59nmpyphbnahxoplk1zq9immrrzm4h01je-.jpg?1528647587&s=b7647bd3f4e69f9d820aeeaa23dd1b04" class = "center" style= "width:400px;height:200px;" ;>
           <img src="https://previews.123rf.com/images/studiostoks/studiostoks1806/studiostoks180600305/103637325-hungry-woman-astronaut-eating-kebab-doner-shawarma.jpg" class = "center" style= "width:400px;height:200px;" ;>

        </header>

            

        <footer>
            <p>&copy;Sam's Mediterranean Grill</p>
        </footer>
   
        </body>
</html>
